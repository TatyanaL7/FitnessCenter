...
class ClientWorkout(models.Model):
    ...
    workout = models.ForeignKey(Workout, on_delete=models.CASCADE, verbose_name="Занятие")
    date = models.DateField(verbose_name="Дата занятия")

    def __str__(self):
        return f"{self.client} - {self.workout} ({self.date})"
